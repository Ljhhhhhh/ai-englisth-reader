...
3. 可选：如果你希望本地 SQLite 结构和种子数据与当前 MVP 保持一致，可执行 Prisma 初始化
...
- 文章内容来自 `content/articles/*.json`，并通过 Zod 做结构校验。
- 阅读进度、生词和学习事件都保存在同一设备的 localStorage 中。
- Prisma 当前使用本地 SQLite 文件，便于轻量开发。
